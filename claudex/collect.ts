...
const LAUNCH_LABEL = "com.contextis.watch";
const RUN_KEY_NAME = "contextis-watch";
...
async function enableAutostart() {
  ...
  if (!target) {
    console.log(
      ...
      "then `contextis login <token> --server <url>` (not `npx tsx`)."
    );
    return;
  }
  ...
  try {
    if (process.platform === "win32") {
      ...
      const regScript =
        ...
        `New-ItemProperty -Path $run -Name '${RUN_KEY_NAME}' -Value $cmd -PropertyType String -Force | Out-Null\r\n`;
